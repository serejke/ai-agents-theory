...
- **Bootstrap the reader before using theory vocabulary.** On first encounter — the README, and the opening paragraphs of each concept document — define the concept using words a reader without prior taxonomy exposure already knows (programs, tasks, tools, filesystems, loops), then tie in the formal name. Don't define Agent as "a Session wrapped in a Frontend and pinned to an Environment" when those are theory words the new reader hasn't met. Lead with what the concept does in everyday terms, anchor with a concrete example, and only then introduce the formal vocabulary (bold the term when tying it in). Deep-body sections may assume taxonomy familiarity; openings may not. A reader who arrives at the document cold should be able to follow the first few paragraphs without clicking any cross-references.
