...
1. **A state machine DSL** — nodes + edges + conditional routing. Maps to [StateMachine](../primitives/state-machine.md).
2. **A shared state system** — typed `TypedDict` flowing between nodes, with reducer functions for concurrent updates. Maps to [Channel (Shared State variant)](../primitives/channel.md).
...
4. **A tool-use loop factory** — `create_react_agent()` builds a two-node graph (call_model ↔ call_tools). Maps to [AgentLoop](../patterns/agent-loop.md).
...
LangGraph's `StateGraph` is a direct implementation of the theory's [StateMachine](../primitives/state-machine.md) primitive:
...
LangGraph uses the [Shared State Object](../primitives/channel.md) variant of Channel exclusively. A `TypedDict` flows between nodes; each node receives the full state and returns a partial update.

The theory identifies [six channel variants](../primitives/channel.md): filesystem, return value, artifact, shared state, mailbox, and git. LangGraph offers one. This creates a real tension with large artifacts.
...
This is the theory's [AgentLoop](../patterns/agent-loop.md) — a recursive cycle of LLM call → tool execution → LLM call. The theory models it as a while loop. Anthropic implements it as a while loop (~30 lines). LangGraph models it as a graph.
...
This is a deliberate trade-off: you get checkpointing without deploying a separate orchestrator, at the cost of coupling orchestration concerns into your agent code. For the class of systems where this bundling works, see the [concern-bundling analysis](../primitives/state-machine.md) in the StateMachine document.

### Memory vs. State — A Clean Separation

LangGraph separates two persistence concepts:

- **Checkpointer** — run-scoped state (conversation history, intermediate results). Maps to [Session](../patterns/session.md) history + [Channel](../primitives/channel.md).
- **Store** (`BaseStore`, `PostgresStore` with pgvector) — cross-thread, cross-session knowledge. Maps to the [Memory](../primitives/memory.md) primitive.
...
LangGraph supports supervisor, swarm, and hierarchical topologies. These recurring structures are now documented in [Deployment: Multi-Agent Topologies](../patterns/deployment.md).
...
LangGraph models `create_react_agent()` as a two-node graph. The theory correctly identifies this as [AgentLoop](../patterns/agent-loop.md) — a recursive cycle that needs no graph. Anthropic's implementation is ~30 lines of imperative code. The graph adds overhead for something that is fundamentally:
...
LangGraph is the theory's [StateMachine](../primitives/state-machine.md) as a product — with [Channel (SharedState)](../primitives/channel.md), checkpointing, and [AgentLoop](../patterns/agent-loop.md) bundled in. It validates the theory's core claim that StateMachine is a fundamental building block. It revealed patterns (handoff, multi-agent topologies, concern-bundling) that are now part of the theory.
...
- [StateMachine](../primitives/state-machine.md) — LangGraph's core primitive, including the graph vs. imperative analysis
- [Channel](../primitives/channel.md) — six variants vs. LangGraph's one
- [AgentLoop](../patterns/agent-loop.md) — while loop vs. graph representation
- [Memory](../primitives/memory.md) — full taxonomy vs. LangGraph's Store
- [Verification](../verification.md) — the full verification stack beyond trace capture
