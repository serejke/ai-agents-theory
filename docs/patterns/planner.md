...
A Planner is a [Session](session.md) configured with read-only tools and a system prompt that requires structured output — a plan, not an implementation. It is the pattern behind every system where the agent first thinks, then acts.

**Composition**: Session + constraint (read-only tools + structured output prompt).

## Why It Matters

Letting an agent immediately start writing code or making changes is risky for non-trivial tasks. The agent may misunderstand the problem, miss important context, or take an approach that contradicts existing architecture. A Planner forces the agent to explore the codebase and propose a structured plan _before_ any writes happen. The plan can be reviewed by a human, by an [Evaluator](evaluator.md), or by a [StateMachine](../primitives/state-machine.md) guard before execution proceeds.
...
This is not a new primitive — it's a Session with a specific configuration (read-only tools + system prompt requiring structured output). Claude Code plan mode, Devin planning phase, Codex reasoning — all implement this pattern.

---

## Planner in Context

Planner typically appears as a phase in a [StateMachine](../primitives/state-machine.md):
...
- [Session](session.md) — the base pattern that Planner constrains
- [StateMachine](../primitives/state-machine.md) — Planner is typically one phase in a multi-phase workflow
